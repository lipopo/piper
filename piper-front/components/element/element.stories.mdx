...
<Preview>
    <Story name='element-contianer'>{{
        ...
        template: `
            <element-container :element='element' :flow='flow'></element-container>
        `,
        mounted: function() {
            this.element = new Element(
                0, 'Entry Element', 'input', {}
            );
            this.flow = new Flow(this.element);
            let new_ele = new Element(null, 'Normal Element', 'echo', {});
            this.flow.link(this.element, new_ele);
        },
        data: () => {
            return {
                element: {},
